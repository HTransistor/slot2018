...
Für Germanium ist die Größe der Bandlücke mit \(E_G=0,67\unit{eV}\) angegeben. Die Boltzmann-Konstante im Gaußschen Einheitensystem mit \(k_b=8,617\cdot 10^{-5}\unit{\frac{eV}{K}}\) ist ebenfalls bekannt. 
\begin{align*}
W(T) = \exp(-\frac{E_G}{2 k_B T})
\end{align*}
...
In der hier durchgeführten Rechnung wird der bereits in Aufg. 1 errechnete Wert der Besetzungswahrscheinlichkeit für \(T=300\unit{K}\)übernommen. Zudem sei \(n_i(T_0=300\unit{K})=2,3\cdot 10^{6}\unit{m^{-3}}\) und \(\sigma_0(T_0=300\unit{K})=2,14\unit{A\cdot(Vm)^{-1}}\) bekannt. Gleichung (3):
\begin{align*}
e\cdot n_i (\mu_n+\mu_p)\approx & \sigma \cdot \exp(-\frac{E_G}{2 k_B T}) \\
\Leftrightarrow \mu_n+\mu_p \approx & \frac{2,14\unit{\frac{A}{V m}}}{2,3\cdot 10^{13}\unit{cm^{-3}}1,602\cdot10^{-19}\unit{C}} = 5807 \unit{\frac{cm^2}{V s}}
\end{align*}
Mit einem Verhältnis der Beweglichkeiten von Löchern und Elektronen von \(\mu_n \approx 2,05 \mu_p\) ergeben sich hier:
\begin{align*}
\mu_p\approx & 1903 \unit{\frac{cm^2}{V s}} \\
\mu_n\approx & 3903 \unit{\frac{cm^2}{V s}}
\end{align*}

\subsection{Ladungsträgerdichte in Metallen}
...
Für den Fit wurde eine Näherung der Steigung  $a=1.1953\times 10^{-19}$ verwendet.\\
...
\subsubsection{Fazit des Vergleichs der Messungen}
...
Wie in Abbildung \ref{fig:M3_Dis_2} zu erkennen sinkt die Leitfähigkeit bei $T=410K$ auf Null und steigt danach wieder an. Weiterhin ist dieses Ansteigen eine signifikante Abweichung von der zu erwartenden Nullmessung, wie es anhand des Fehlerbalkens zu sehen ist.\\
\\
